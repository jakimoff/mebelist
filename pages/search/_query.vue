<template lang="pug">
  div
    BreadcrumbsSimple
    TitlePage(:title='`Результаты поиска по запросу "` + $nuxt.$route.params.query + `"`')
    .container
      .row
        .col-xl-3.d-none.d-xl-block
          aside.aside-page__aside
            .catalog-sub--wrap
              .catalog-sub
                ul._list.list-result
                  li._item(v-for="item in categories")
                    nuxt-link(:to="item.to")._link.text_14
                      | {{ item.title }} ({{ item.count }})
                Filters(:filters="filters" :filter="filter" @select-filter="setFilter")
        .col-xl-9.col-lg-12
          .aside-page__content
            .sort--wrap
              .sort
...
export default {
  ...
  async mounted() {
    const { total, products, categories, filters } = await this.$store.dispatch('search/setSearchResult', { query: this.$route.params.query })
    this.total = total
  ...
  methods: {
    async load_more() {
      const { products } = await this.$store.dispatch('search/setSearchResult',
        {
